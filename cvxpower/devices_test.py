...
class StaticTest(unittest.TestCase):
    ...
    def test_three_buses(self):
        ...
        network = Group(
            [load1, load2, gen1, gen2, line1, line2, line3], [net1, net2, net3]
        )
        network.optimize(solver="ECOS")
...
class DynamicTest(unittest.TestCase):
    ...
    def test_thermal_load(self):
        temp_amb = (np.sin(np.pi * np.arange(T) / T) + 1e-2).reshape(
            -1, 1
        ) ** 2 * 50 + 50
...
class ScenarioTest(unittest.TestCase):
    def test_dynamic_load(self):
        ...
        np.testing.assert_allclose(
            net.price[:, 0], p_load_robust[:, 0] * 200 + 100, rtol=1e-2
        )
